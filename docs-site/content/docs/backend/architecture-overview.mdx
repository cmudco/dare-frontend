...
See [Socket.IO Event Contract](/docs/backend/socketio-events) for the complete event reference.
...
See `SocraticBooks-DARE Proxy Contract (planned)` for the complete reference.
