...
description: "Production and server deployment: Docker and bare-metal paths, reverse proxy, and troubleshooting."
---
> **Just want to run it locally?** See [QUICKSTART.md](/docs/backend/quickstart) — the **Docker** path stands up
...
For environment variable reference, see [docs/configuration.md](/docs/backend/configuration).
...
The web container waits for Postgres and Redis, then runs migrations before starting Uvicorn. Static and media files are stored in named Docker volumes. See `docs/local-setup-notes.md` for local-dev gotchas (email verification, wallet credit, model catalog).
...
For more, see [docs/admin-guide.md](/docs/backend/admin-guide).
