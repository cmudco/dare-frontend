...
See [docs/architecture.md](/docs/backend/architecture) for the full diagram and [docs/architecture/overview.md](/docs/backend/architecture-overview) for component-level detail.
...
The API will be available at `http://localhost:8000/`. The OpenAPI schema is served at `/api/schema/`. Swagger UI is routed at `/api/docs/`, but it loads Swagger assets from a CDN, so use the raw schema if the UI does not render in an offline or restricted network.

Docker Compose starts the API server, RQ worker, Postgres + pgvector, Redis, and Weaviate. Optional Ollama and Weaviate console services are available through Compose profiles. See [INSTALL.md](/docs/backend/install) for details.
...
Redis must be running for Socket.IO pub/sub and background jobs. See [INSTALL.md](/docs/backend/install) for complete Docker, local, and production guidance.

## Documentation

| Doc                                                                                          | What's in it                                                    |
| -------------------------------------------------------------------------------------------- | --------------------------------------------------------------- |
| [INSTALL.md](/docs/backend/install)                                                                     | Full deployment guide — Docker and bare metal                   |
| [docs/configuration.md](/docs/backend/configuration)                                               | Every environment variable, with type, default, and description |
| [docs/architecture.md](/docs/backend/architecture)                                                 | Component diagram and request flows                             |
| [docs/admin-guide.md](/docs/backend/admin-guide)                                                   | User/role management, access codes, analytics                   |
| [CONTRIBUTING.md](/docs/backend/contributing)                                                           | Issues, pull requests, coding standards                         |
| `CHANGELOG.md`                                                                 | Release notes                                                   |
| `SECURITY.md`                                                                   | Vulnerability disclosure process                                |
| `docs/integration/socraticbooks-dare-proxy.md` | DARE/SocraticBooks integration contract and update rules        |
| [docs/architecture/socketio-events.md](/docs/backend/socketio-events)                 | Socket.IO event reference                                       |
| [docs/api/dare-backend.md](/docs/backend/api-reference)                                         | REST API reference                                              |
| [docs/code-standards.md](/docs/backend/code-standards)                                             | Coding conventions                                              |
