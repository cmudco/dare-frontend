...
See [docs/architecture.md](/docs/backend/architecture) for the full diagram and
[docs/architecture/overview.md](/docs/backend/architecture-overview) for component-level detail.
...
`/api/schema/`. Swagger UI is routed at `/api/docs/`, but
...
[INSTALL.md](/docs/backend/install) for details.
...
Redis must be running for Socket.IO pub/sub and background jobs. See [INSTALL.md](/docs/backend/install) for
...
- `socraticbooks-backend` and
  `socraticbooks-react` — the SocraticBooks platform that
  proxies to DARE
...
Mellon University. See [BRAND.md](/docs/reference/brand) for the brand usage policy. For trademark, licensing, or
