...
description: "Run the frontend locally: install, set two env vars, and start the dev server."
---
Get the frontend running locally in about two minutes. This is all you need for development.
For deploying to a server, see [DEPLOYMENT.md](/docs/frontend/deployment).

## Prerequisites

- **Node.js 18+** (20 LTS recommended)
- A **running DARE backend** — defaults assume `http://localhost:8000` (see `../dare-backend`)
...
`VITE_WEBSOCKET_URL` in `.env` at it. Full variable reference: [docs/configuration.md](/docs/frontend/configuration).
