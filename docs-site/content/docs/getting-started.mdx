...
---
This guide walks you through setting up the full local workspace from scratch: two backends and two frontends. If you only need the DARE API, use the shorter Docker or local Python path in the repository [README](/docs/backend/readme).
...
- Swagger UI: /api/docs/ (requires CDN assets)
...
- [Socket.IO Event Contract](/docs/backend/socketio-events) — Real-time event reference
- [OpenAPI Schema](/api/schema/) — Raw API schema (when backend is running)
- [Swagger UI](/api/docs/) — Interactive API explorer if CDN assets are reachable
- `Inter-Service Proxy` — How SocraticBooks communicates with DARE
