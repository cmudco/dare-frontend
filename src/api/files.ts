...
export const checkJobStatusesAPI = async (fileIds: number[]): Promise<{ fileId: number; status: string; jobId?: string; statusCode: FileStatus; jobStatus: string }[]> => {
    return await baseRequest<{ fileId: number; status: string; jobId?: string; statusCode: FileStatus; jobStatus: string}[]>({
