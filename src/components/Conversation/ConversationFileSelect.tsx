...
import { updateSelectedFiles, updateSelectedTags } from "@/redux/conversationSlice";
...
const ConversationFileSelect: React.FC = () => {
  ...
  const selectedFiles = useSelector((state: RootState) => state.conversation.selectedFiles);
  const selectedTags = useSelector((state: RootState) => state.conversation.selectedTags);
  ...
  return (
    <div className="absolute left-3 flex items-center h-full">
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent className="w-[400px] p-4" align="start" side="top" sideOffset={16}>
          <div className="space-y-4">
            ...
            <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "files" | "tags")}>
                    ...
                    return (
                      <div
                        ...
                        className={`flex items-center p-2 rounded-md ${
                          hasFiles ? "hover:bg-muted cursor-pointer" : "opacity-50 cursor-not-allowed"
                        }`}
                      >
                        <div
                          className={`w-5 h-5 rounded border-2 flex items-center justify-center mr-3 ${
                        ...
                        <span className={`text-sm ${isSelected ? `text-${getTagColor(tag.label)}-600` : ""}`}>
                          {tag.label}
                        </span>
