...
const Sidebar = () => {
  ...
  return (
    <div
      className={`relative h-full flex ${isCollapsed
          ? "w-[80px]"
      ...
      <nav className='flex flex-col gap-1 p-2 font-sans text-base font-normal text-blue-gray-700 flex-grow'>
        {menuItems.map((item) => (
          <Link
            ...
            className={`flex items-center w-full p-3 leading-tight transition-all rounded-xl outline-none text-start ${item.disabled
          ...
          >
            <item.icon
              className={`w-5 h-5 font-bold ${isCollapsed ? "mx-auto" : "mr-4"
                }`}
            />
