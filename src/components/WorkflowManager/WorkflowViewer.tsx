...
import { WorkflowViewerProps, WorkflowMode } from '@/redux/types/workflow'
import {
...
const WorkflowViewer: React.FC<WorkflowViewerProps> = ({
  ...
  const { selectedWorkflowRun, error, workflows } = useSelector(
  ...
  const getIconForMode = () => {
    if (selectedWorkflowRun?.workflow) {
      const workflowId = selectedWorkflowRun.workflow
      const actualWorkflow = workflows.find((w) => w.id === String(workflowId))

      if (actualWorkflow) {
        const modeDetails = WORKFLOW_MODES.find(
          (m) => m.id === actualWorkflow.mode
        )
        if (modeDetails?.id === WorkflowMode.Serial) {
          return <ListOrdered className='mr-2 h-5 w-5 text-gray-700' />
        } else if (modeDetails?.id === WorkflowMode.Parallel) {
  ...
  const handleExportResults = async () => {
    ...
    link.href = url
    link.download = `${workflowName || 'workflow'}-results.pdf`
              ...
              <div className='flex items-center justify-between'>
                <div className='flex items-center'>
                  ...
                  <DrawerTitle className='text-lg font-semibold text-gray-900'>
                    {workflowName ? `${workflowName} - ${title}` : title}
                  </DrawerTitle>
                </div>
                <DrawerClose asChild>
                  <Button
...
export default memo(WorkflowViewer, (prevProps, nextProps) => {
  return (
