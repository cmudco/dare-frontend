...
import type { Workflow, Step as ApiStep, WorkflowRun, WorkflowRunStep } from '../types/workflow'
  ...
  async (workflowId: string, { dispatch }) => {
    ...
    const stepNodes: Node[] = workflowSteps.map((step: ApiStep, idx: number) => {
      const stepData = {
        ...
        maxContextSnippets: typeof step.maxContextSnippets === 'number' ? step.maxContextSnippets : 4,
        documentSimilarityThreshold: typeof step.documentSimilarityThreshold === 'number'
          ? step.documentSimilarityThreshold : 0.2,
